package com.jap.course3;

public class SavingAccount extends BankAccount {
    private int age;
    private int timeperiod;
    SavingAccount(){
        this("",0,0,0);
    }
    SavingAccount(String name,double balance,int a,int t){
        super(name,balance);
        age=a;
        timeperiod=t;
    }
    public void setAge(int a) {
        age=a;
    }
    public int getAge() {
        return age;
    }
    public void setTime(int t) {
        timeperiod=t;
    }
    public int getTime() {
        return timeperiod;
    }
    public void setStatus() {
        if(super.getcountOfTransactions()>=1) {
            super.status="active";
        }
        else {
            super.status="inactive";
        }
    }
    public void debit(double amount) {
        if(super.getAccountBalance()-amount<super.getminimumbalance()) {
            System.out.println("Debit Not Possible as it is conflicting with your minimum balance ");
        }
        else if(amount>super.getAccountBalance()) {
            System.out.println("Enter proper amount");
        }
        else {
            super.setAccountBalance(super.getAccountBalance()-amount);
            setStatus();
            System.out.println("Debited successfully!");
        }
    }
    public void credit(double amount) {
        if(amount>40000) {
            System.out.println("Amount is too high to credit.It's exceeding credit limit");
        }
        else {
            super.setAccountBalance(super.getAccountBalance()+amount);
            super.setcountOfTransactions(super.getcountOfTransactions()+1);
            setStatus();
            System.out.println("Credited successfully!");
        }
    }
    public void setAccountType() {
        System.out.println("Welcome to savings account");
        super.accountType="savings";
    }
    public String toString() {
        return "Account number : "+super.getaccnumber()+"\n"+"Account Holder Name : "+super.getaccountHolderName()+
                "\n"+"Account Type : "+super.getAccountType()+"\n"+"Account Balance : "+super.getAccountBalance()+
                "\n"+"Account Status : "+super.status+"\n Account Time Period : "+getTime();
    }
}


